...
void solve_ronit(int tc) {
    ...
    int count_less_than_min = 0;
    int count_more_than_min_less_than_max = 0;
    int count_more_than_max = 0;

    for (auto &&i : a)
    {
        if (i <= min_) {
            count_less_than_min++;
        } else if (i > min_ && i <= max_) {
            count_more_than_min_less_than_max++;
        } else {
            count_more_than_max++;
        }
    ...
    int ans = 0;
    if(count_more_than_min_less_than_max >= count_less_than_min)
    {
        ans += count_less_than_min;
    ...
    {
        ans += count_more_than_min_less_than_max;
        count_less_than_min -= count_more_than_min_less_than_max;
        ans += count_less_than_min / 2;

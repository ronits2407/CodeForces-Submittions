...
void solve_ronit(int tc) {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (a[i - 1] < mex_array[i - 1])
        {
            mex_array[i] = mex_array[i - 1];
        }
        else
        {
            int mex = mex_array[i - 1];
            while (freq[mex] > 0) {
                mex++;
            }
            mex_array[i] = mex;
    ...
    // give(mex_array, n + 1);
    // give(suffix_mex, n + 1);
    // check if for any point mex of prefix is same as suffix
    bool ans = true;
    for (int i = 1; i <= n; i++) {
        if (mex_array[i] == suffix_mex[i]) {
            ans = false;
            break;
        }
    }
